...
################################################################################
#             
# Script:    procmm.sh (Process Memory Matrix) v1.02     
#             
...
get_matrix() {
    ...
    paste /tmp/procmm-x.$$ /tmp/procmm-s.$$ | sed 's/dism shmid/dism_shmid/g' | tr '[]' '  ' | egrep -v "^ *Address|^---------|total Kb|^[0-9]*:" | nawk '
    {
        item=$7
        #print "before", s
        sub(/.*\.so.*/, "lib", item)
	sub(/.*dev:.*/, "devices",item)
        #print "after", s
...
if [ $compute_total -eq 0 ]; then 
    for pid in $pidlist; do
        ...
        pmap -x$pmap_option $pid > /tmp/procmm-x.$$ 
        pmap -S$pmap_option $pid > /tmp/procmm-s.$$ 
        get_matrix $pid
#        rm -f /tmp/procmm-x.$$ /tmp/procmm-s.$$
    done
else
#    rm -f /tmp/procmm-x.$$ /tmp/procmm-s.$$
   ...
   echo "-- totals only as other numbers may be heavily \"doublecounted\" due overlaps of shared mappings\n"
#   rm -f /tmp/procmm-x.$$ /tmp/procmm-s.$$
 fi

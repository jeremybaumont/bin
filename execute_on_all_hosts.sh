...
##
typeset  __SCRIPT_VERSION="v0.0.4"
##
...
__LONG_USAGE_HELP='
      ...
      -u sshuser
              sshuser is the userid to use on the target host
...
##
__SHORT_USAGE_HELP='                    -i hostlist{,hostlist1{,...} -s scriptfile [-o outputfile] [-u userid] [-c shell] [-k] [-U] [-I basedir]
...
#
  ...
  __GETOPTS="+:ynvqhHDfl:aOS:CVTi:o:s:u:c:kUI:"
  if [ "${__OS}"x = "SunOS"x  ] ; then
    if [ "${__OS_VERSION}"x  = "5.10"x -o  "${__OS_VERSION}"x  = "5.11"x ] ; then
      __GETOPTS="+:y(yes)n(no)v(verbose)q(quiet)h(help)H(doc)D(debug)f(force)l:(logfile)a(color)O(overwrite)S:(summaries)C(writeconfigfile)V(version)T(tee)i:(hostlist)o:(outputfile)s:(scriptfile)u:(sshuser)c:(shell)k(nocomments)U(uniquelogfiles)I:(basedir)"
    fi
  fi
...
# check SSH agent
  ...
  for CUR_HOSTFILE in  ${HOSTFILE_LIST} ; do
    for CURMACHINE in $( cat "${CUR_HOSTFILE}" | tr -d "\r" | tr -s " " | grep -v "^#" | grep -v "^$" ) ; do
      if [ ${UNIQUE_LOGFILES} = ${__FALSE} ] ; then
      ...
      LogMsg  "---- Processing \"${CURMACHINE}\" ..."
      executeCommand ${SCP_BINARY} "${SCRIPTFILE}" "${SSHUSER}@${CURMACHINE}:${TARGET_TMPFILE}"
      if [ $? -ne 0 ] ; then
      ...
      if [ ${ADD_COMMENTS} = ${__TRUE} ] ; then
        echo "# ### ---- Log of the script executed on host \"${CURMACHINE}\" --- start ---" >>"${CUR_OUTPUTFILE}"
      fi
    
      executeCommandAndLog ${SSH_BINARY} -l "${SSHUSER}" "${CURMACHINE}" ${SHELL_TO_USE} ${TARGET_TMPFILE} | tee -a "${CUR_OUTPUTFILE}"
